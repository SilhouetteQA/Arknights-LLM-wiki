...
监控数据在每次会话总结中汇总，写入 `output/sessions/YYYY-MM-DD.md` 的数据基线段落。
...
| **会话总结** | `output/sessions/YYYY-MM-DD.md` | 本次会话对话轮次、每轮解决内容、修改文件清单、遗留问题、下一步计划 | 每次会话结束时 |
...
1. Hook 自动提示保存
2. 写 `output/sessions/YYYY-MM-DD.md`（对话轮次+解决内容+修改清单+遗留问题+下一步）
