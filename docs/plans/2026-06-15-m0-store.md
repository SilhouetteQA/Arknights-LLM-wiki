...
- [x] **Step 1: 写 EntityRepository 测试**
...
- [x] **Step 1: 写 SourceIndexRepository 测试**
...
- [x] **Step 3: 实现 SourceIndexRepository**
...
- [x] **Step 1: 写 WikiPageRepository 测试**
...
- [x] **Step 3: 实现 WikiPageRepository**
...
- [x] `pytest tests/test_store.py -v` 所有测试通过
- [x] `python -m arknights_wiki.store.seed` 无报错执行
- [x] `data/arknights_wiki.db` 存在，entities 表 character > 400, faction > 0, region > 0
- [x] entity_aliases 表有 40 条以上的异格/升变别名
- [x] source_index 表有大量条目（exact + concept_keyword）
- [x] `output/m0_entities_by_chapter/` 每个章节都有对应 md
- [x] 无名 NPC（如 整合运动成员、罗德岛干员）不出现在 entities 表中
