...
本文件不 import 任何 ``arknights_wiki`` 业务模块。
"""
...
def test_package_discovery_covers_project_and_agent_core() -> None:
    """显式 package discovery 必须同时覆盖本项目包与 agent_core。"""
    include = _pyproject()["tool"]["setuptools"]["packages"]["find"]["include"]
    assert "agent_core*" in include, f"agent_core 未进入 package discovery：{include}"
    assert "arknights_wiki*" in include, f"本项目包未进入 package discovery：{include}"
...
@contract_rule("FND-PKG-003")
def test_wheel_ships_the_project_package_and_nothing_else(wheel_entries: list[str]) -> None:
    """wheel 顶层只允许本项目包、agent_core 与 dist-info；不得夹带仓库其他目录。"""
    tops = {entry.split("/")[0] for entry in wheel_entries}
    allowed_tops = {"arknights_wiki", "agent_core"}
    unexpected = {
        top
        for top in tops
        if top not in allowed_tops and not top.endswith(".dist-info")
    }
    assert not unexpected, f"wheel 夹带了未声明的顶层内容：{sorted(unexpected)}"
    assert "arknights_wiki/__init__.py" in wheel_entries
