"""Spec 10 工具开发验证：`replay_history.py` / `publish_evidence.py`（Wiki 仓）。
...
def _registry(repository: str) -> dict:
    return {
        ...
        "producers": [
            {
                "producer_id": f"{repository}.agent.llm_usage",
                "mapping_stages": ["chat_completion"],
                "status": "IN_SCOPE",
                "source_locations": [],
                "legacy_fields": ["tokens_in", "tokens_out", "cost", "model"],
                "foundation_objects": ["Usage", "Cost"],
                "evidence_requirement": "ALL_STAGES",
            },
            {
                "producer_id": f"{repository}.extraction.pass1",
                "mapping_stages": [],
                "status": "DEFERRED",
                "source_locations": [],
                "legacy_fields": [],
                ...
                "reason": "offline_batch_lifecycle_out_of_v0.1_scope",
            },
        ],
    }
...
def _write_sources(root: Path, repository: str) -> None:
    ...
    lines = [
        {
            "step": "judge",
            "model": "model-absent-from-pricing",
            "tokens_in": 631,
            "tokens_out": 775,
            "cost": 0.0006543,
        },
        {"step": "judge", "model": "model-absent-from-pricing", "cost": 0.5},
        # 既无 token 也无 cost：必须判 LEGACY_DATA_INSUFFICIENT
        {"step": "judge", "note": "no usage facts"},
    ]
    (eval_dir / "cost_log.jsonl").write_text(
        "\n".join(json.dumps(item) for item in lines) + "\n", encoding="utf-8"
    )
    # 第二个来源用 JSON 数组形态，覆盖 `_iter_records` 的分支
    (eval_dir / "results.json").write_text(
        json.dumps([{"prompt_text": "raw", "tokens_in": 7, "cost": 0.01}]), encoding="utf-8"
    )
...
def _replay_manifest(root: Path, repository: str, payload_hash: str) -> dict:
    return {
        ...
        "sources": [
            {
                "source_id": "src-cost-log",
                "source_class": "cost_log",
                "path": "output/eval/cost_log.jsonl",
                "producer_id": f"{repository}.agent.llm_usage",
...
def _smoke_manifest(repository: str, payload_hash: str) -> dict:
    return {
        ...
        "provider": "provider-x",
        "case_ids": ["case-1"],
        "expected_calls": {f"{repository}.agent.llm_usage": 1},
        "max_calls": 4,
        "estimated_cost_cap": {"amount": "1", "currency": "CNY"},
...
def _build_repo(tmp_path: Path, repository: str = "wiki") -> dict:
...
def test_unknown_producer_and_stage_mismatch_are_usage_errors(tmp_path):
    ...
    for source_index, mutate, fragment in (
        (
            0,
            lambda s: s.update({"producer_id": "wiki.ghost.producer"}),
            "不在本仓 producer registry",
        ),
        (0, lambda s: s.update({"mapping_stage": "judge"}), "登记 stage"),
        (1, lambda s: s.update({"runtime_adapter_status": "IN_SCOPE"}), "runtime_adapter_status"),
    ):
...
def test_deferred_source_requires_deferred_status_and_null_stage(tmp_path):
    repo = _build_repo(tmp_path)
    manifest = json.loads(repo["manifest_path"].read_text(encoding="utf-8"))
    manifest["sources"][1]["mapping_stage"] = "chat_completion"
    repo["manifest_path"].write_text(json.dumps(manifest), encoding="utf-8")
...
def test_source_file_with_bom_fails_with_exit_code_1(tmp_path):
    repo = _build_repo(tmp_path)
    (tmp_path / "output" / "eval" / "cost_log.jsonl").write_bytes(
        b"\xef\xbb\xbf" + (tmp_path / "output" / "eval" / "cost_log.jsonl").read_bytes()
    )
    with pytest.raises(rh.ToolError) as excinfo:
        _run(tmp_path, repo)
...
def test_mapping_failure_is_recorded_per_record_not_fatal(tmp_path, monkeypatch):
    ...
    assert len(outcome.corpus_records) == 4
...
def _publish(tmp_path, monkeypatch, *, repository="wiki", dry_run=False, candidate=COMMIT, **kwargs):
...
def test_allowlist_is_an_explicit_allowlist_including_changelog():
    allowlist = pe.release_allowlist("0.1.0", "wiki")
    assert "docs/contracts/releases/0.1.0/changelog.md" in allowlist  # G-08 显式收敛
    assert "docs/contracts/releases/0.1.0/validation/wiki/evidence-manifest.json" in allowlist
...
def test_publish_writes_only_allowlisted_paths_under_tmp(tmp_path, monkeypatch):
    ...
    assert written == sorted(pe.release_allowlist("0.1.0", "wiki"))

    evidence_manifest = json.loads(
        (release_dir / "validation" / "wiki" / "evidence-manifest.json").read_text(encoding="utf-8")
    )
    assert evidence_manifest["repository"] == "wiki"
    assert evidence_manifest["evidence"]["fresh_smoke"] == "NOT_OBSERVED"
    assert "evidence_manifest_hash" not in evidence_manifest

    run_manifest = json.loads(
        (release_dir / "validation" / "wiki" / "run-manifest.json").read_text(encoding="utf-8")
    )
    assert run_manifest["candidate_commit"] == COMMIT
    assert run_manifest["commands"]

    corpus_lines = (
        release_dir / "validation" / "wiki" / "sanitized-replay-corpus.jsonl"
    ).read_text(encoding="utf-8").splitlines()
    assert corpus_lines
    for line in corpus_lines:
        assert set(json.loads(line)) == set(rh.CORPUS_RECORD_KEYS)

    report = (release_dir / "validation" / "wiki" / "validation-report.md").read_text(
        encoding="utf-8"
    )
...
def test_require_stages_must_match_registry(tmp_path):
    ...
    smoke["required_producer_stages"].append(
        {"producer_id": "wiki.ghost", "mapping_stage": "x", "evidence_requirement": "ALL_STAGES"}
    )
...
#: 本仓 registry 中 `status == "IN_SCOPE"` 的 producer / stage 组合数（Wiki: 3 / 6）。
IN_SCOPE_PRODUCERS = 3
IN_SCOPE_STAGES = 6
