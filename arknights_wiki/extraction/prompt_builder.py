...
4. **power_level（战力评估体系）**：
   九级战力框架（由低到高），根据角色在剧情中的战斗表现选择最匹配的等级：

   1. 信息不足：没有直接战斗描写，或者战斗能力不足以以一敌多
   2. 战场中坚：能以一敌多，基准角色如萨卡兹雇佣兵、碎骨、弑君者、霜叶
   3. 军事精锐：各国精锐士兵，基准角色如银枪天马、教宗铳骑、金律法卫、塞雷娅、风笛、黑蓑影卫、普通灰礼帽、赫德雷
   4. 大国将军：具有特殊能力的战场顶尖战力/指挥官，基准角色如曼弗雷德、罗德岛部分精锐干员、全装临光、闪灵、莫斯提马
   5. 传奇英雄：具有一己之力扭转战场局势的能力，基准角色如黑蛇塔露拉、圣徒卡门、凯尔希、深海猎人、槐天裴、西里尔·临光、雪原时期的爱国者
   6. 王庭之主：萨卡兹各族之主，具有超乎想象的能力；此外伦蒂尼姆时期的阿米娅也属此级
   7. 神明碎片：具有概念性质的能力，基准角色如年纪较小的岁兽代理人、巫王、双子女皇
   8. 崛起之物：比肩神明，一人成军，影响一方，基准角色如年纪较大的岁兽代理人、岁兽、老天师、耶拉冈德、伊莎玛拉
   9. 文明之敌：灭世级灾厄能力，如伊祖米克、无垠回荡克雷松、祖灵之父萨米、源石原初天灾；主要出现在水月/萨米/岁/萨卡兹集成战略的后面几个结局

   格式为等级名，如「战场中坚」「传奇英雄」。无法确定时使用「信息不足」。

5. **personality.traits**：2-5 个简短的性格标签词，如「坚毅」「腹黑」「忠诚」。

6. **abilities.description**：一句话概括角色的核心战斗能力或特殊技能。

	7. **abilities.power_level_evidence**：列出支撑 power_level 评级的 1-3 个关键战斗事件，每项包含 chapter（章节名）和 evidence（该事件中展现战力水平的简述）。必须引用对话中实际出现的战斗场景，不编造。"""

CHARACTER_OUTPUT_SCHEMA = """```json
{
  "summary": "跨章角色总结（基于提供的对话内容，不编造）",
  "personality": {"traits": ["标签1", "标签2"], "description": "1-2句性格描述"},
  "abilities": {"description": "一句话能力概括", "power_level": "战场中坚 或 信息不足", "power_level_evidence": [{"chapter": "章节名", "evidence": "该事件中展现战力水平的简述"}]},
...
def build_character_user_prompt(
    ...
    parts.append("- power_level 按九级战力体系评估，格式为等级名（如「战场中坚」），不确定时使用「信息不足」")
